...
import { Target, ArrowLeft, Check, AlertCircle, Loader2, Mic, Save, Search } from 'lucide-react';
import { useAuthContext } from '../components/AuthProvider';
import { getAllSkills, getUserSkills, getSpeechSkills, updateSpeech, updateSpeechSkills, getUserSpeeches, Skill, UserSkill, Speech } from '../lib/supabase';
...
const SpeechEdit: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (currentSpeech.user_id !== user.id) {
        setError('You do not have permission to edit this speech');
        return;
      }
      ...
      // Load skills data
      const [allSkills, userSkillsData, speechSkills] = await Promise.all([
        getAllSkills(),
      ...
      setSpeechData({
        ...
        description: currentSpeech.description || '',
        selectedSkills: speechSkills.map(ss => ss.skill_id)
      });
  ...
  const handleSubmit = async () => {
    if (!speech || !user?.id) {
      setError('Invalid speech or user data');
      return;
    }
    ...
    try {
      // Update the speech record
      console.log('Updating speech:', speechData);
      
      await updateSpeech(speech.id, {
        title: speechData.title.trim(),
        description: speechData.description.trim() || undefined
      });
      
      console.log('Speech updated successfully');
      
      // Update speech-skill associations
      await updateSpeechSkills(speech.id, speechData.selectedSkills);
      console.log('Speech skills updated successfully');
      
      // Navigate back to speeches page
      navigate('/speeches');
    } catch (error) {
      console.error('Error updating speech:', error);
      setError('Failed to update speech. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <div className="space-y-8">
        <div className="bg-white rounded-xl p-8 shadow-sm border border-slate-200 text-center">
          ...
          <p className="text-slate-600 mb-6">The speech you're trying to edit could not be found.</p>
          <button
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h2 className="text-3xl font-bold text-slate-800">Edit Speech</h2>
          <p className="text-slate-600">Update your speech details and practice skills</p>
        </div>
      </div>
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-6">
            <div className="text-center">
              ...
              <p className="text-slate-600">Update your speech information and practice skills</p>
            </div>
          ...
          <div className="space-y-6">
            <div className="text-center">
              ...
              <h3 className="text-xl font-bold text-slate-800 mb-2">Update Skills to Practice</h3>
            ...
            >
              {submitting ? (
                <>
                  ...
                  <span>Updating Speech...</span>
                </>
              ) : (
                <>
                  <Save className="w-4 h-4" />
                  <span>Update Speech</span>
                </>
              )}
            </button>
          </div>
